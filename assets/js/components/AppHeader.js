...
window.DangoeulComponents.AppHeader = {
  ...
  <header class="glass" style="height:var(--header-h);display:flex;align-items:center;padding:0 20px;gap:14px;position:sticky;top:0;z-index:50;border-left:none;border-right:none;border-top:none;">
    ...
    <button @click="navigate('home')" style="background:none;border:none;cursor:pointer;display:flex;align-items:center;gap:10px;flex-shrink:0;padding:0;">
      <img v-if="config.brandLogo" :src="config.brandLogo" :alt="config.name + ' 로고'" width="40" height="40" style="width:40px;height:40px;object-fit:contain;flex-shrink:0;display:block;" />
      <div v-else style="width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,var(--blue),var(--green));display:flex;align-items:center;justify-content:center;font-size:1rem;">🌾</div>
